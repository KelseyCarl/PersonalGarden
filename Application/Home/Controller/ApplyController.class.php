...
//                $filePath = "./Public/img_up/".$file["name"];//获取文件原名
                ...
                if ($err !== null) {
                    ...
                    //将用户提交的售卖信息入库
                    $sql = "select max(item_id) as last_item from items";
                    $result = $Model->query($sql);
//                    echo $result[0]["last_item"]."  ";
//                    echo $result[0]["last_item"]+1;
                    $max_item = $result[0]["last_item"]+1;
                    //售卖人信息查询
                    $sql2 = "select nickname from userinfor where user_phone='$username'";
                    $re2 = $Model->query($sql2);
                    $from = $re2[0]["nickname"];
//                    $status_desc = ""
                    $time = date("Y-m-d H:i:s");
                    //入库操作
                    $sql_insert = "insert into items(item_id,item_name,item_desc,item_from,item_amount,item_sale_out,item_img,is_verify,apply_time,item_type,type_desc)
                    ...
                    $update = "update userinfor set apply_sale=apply_sale+1 where user_phone='$username'";
    ...
    public function rent(){//租赁申请
        ...
        switch($this->_method) {
            case 'post':{
                ...
                $sql_query = "select balance from wallet where token='$token'";
...
//                    $this->response(retmsg(-2,null,"余额已不足，无法提交申请"),'json');
                    ...
                    $time = date("Y-m-d H:i:s");
                    $up_wallet = "update wallet set balance=balance-'$money',pay_time='$time' where token='$token'";
                    $Model->execute($up_wallet);
                    $sql_insert = "insert into rent_soil(user_phone,soil_id,farm_belong,soil_status,rent_period)
                  values (" ."'". safe($user_phone) . "','" .
                        safe($soil) . "','" . safe($farm) . "'," . safe("0") .",'".safe($period)."'". ")";
                    $Model->execute($sql_insert);
                    //修改土地名称
                    $up_soil = "update soil set soil_name='$nickname',rent_to_who='$token',soil_desc='$desc' where soil_id='$soil'";
                    $Model->execute($up_soil);
                    $area = "select soil_area from soil where soil_id='$soil' ";
                    $area_re = $Model->query($area);
                    $area1 = $area_re[0]["soil_area"];
                    $in = "update rent_soil set soil_area='$area1'where soil_id='$soil'";
                    $re_in = $Model->execute($in);
                    $update = "update userinfor set soil_nums=soil_nums+1 where user_phone='$user_phone'";
                    $Model->execute($update);
                    if(is_bool($re_in)){
                        $this->response(retmsg(-1,null,"申请提交失败"),'json');
                    }else{
    ...
    public function query_rent($token){//租赁申请记录查询
        ...
        switch($this->_method) {
            case 'get':{
                ...
                $sql2 = "select * from rent_soil where user_phone='$user'";
                $re = $Model->query($sql2);
                $data = array();
                for($i = 0;$i < count($re);$i++){
                    $test1 = array("rent_period"=>$re[$i]["rent_period"]);
                    $item_id = $re[$i]["soil_id"];
                    $sql3 = "select * from soil where soil_id='$item_id'";
...
//                    $data["subitem"][] = $re2;
    ...
    public function query_sale($token){//售卖申请记录查询
        ...
        switch($this->_method) {
            case 'get':{
                $sql = "select nickname from userinfor where token='$token'";
                $result = $Model->query($sql);
    ...
    public function transfer($token){//转让:转让土地
        ...
        switch($this->_method) {
            case 'get':{
                $sql = "select * from soil where soil_status=1 and rent_to_who='$token'";
                $result = $Model->query($sql);
                if(is_bool($result)){
                    $this->response(retmsg(-1,null,"查询失败"),'json');
                }else{
    ...
    public function trans(){//土地转让
        ...
        switch($this->_method) {
            case 'post':{
                ...
                $soil = $_POST["soil"];
                $time = date("Y-m-d H:i:s");
                $insert = "insert into transfer(trans_out,soil_id,trans_in,trans_time) values("."'".safe($token1)."','".safe($soil)."','".safe($token2)."','".safe($time)."'".")";
    ...
    public function query_transfer($token){//转让申请记录查询
        ...
        switch($this->_method) {
            case 'get':{
                $sql2 = "select * from soil where status_desc='转让待租赁' and rent_to_who='$token'";
                $re = $Model->query($sql2);
                $data = array();
