    ...
    public function all_soil($token="",$belong="",$is_rent="",$page="1",$pagesize="10"){
        ...
        switch($this->_method) {
            case 'get':{
                ...
                $l = ($page-1)*$pagesize;
                if($where == "") {
                    $sql1 = "select * from soil order by soil_id desc limit ".$l.",".$pagesize;
                }else{
                    $sql1 = "select * from soil  where  ".$where."  order by soil_id desc  limit ".$l.",".$pagesize;
//                    $m='1';
                }
                ...
                for($i = 0;$i < count($list);$i++){
                    ...
                    $return = array("sid"=>$list[$i]["sid"],"soil_id"=>$list[$i]["soil_id"],"soil_photo"=>$list[$i]["soil_photo"],
                        ...
                        "farm_belong"=>$list[$i]["farm_belong"],);
                    $data[] = $return;
                }
                if(is_bool($list)){
    ...
    public function my_soil($token="",$id){//用户的id
        ...
        switch($this->_method) {
            case 'get':{
                $sql1 = "select user_phone from userinfor where user_id='$id'";
                $re1 = $Model->query($sql1);
                $phone = $re1[0]["user_phone"];
                $sql2 = "select * from rent_soil where user_phone='$phone'";
    ...
    //            $json = json_decode($data,true);
                ...
                $up = "update rent_soil set soil_status='$rent_status',verify_status='$verify_status',verify_desc='$desc',unpass_cause='$unpass',verify_time='$time' where rent_id='$rent_id'";
                $result = $Model->execute($up);
                //获取土地编号
                $sql = "select soil_id from rent_soil where rent_id='$rent_id'";
                $reslut2 = $Model->query($sql);
                $soil_id = $reslut2[0]["soil_id"];
                //更新土地信息表
                $up2 = "update soil set soil_status='$rent_status',status_desc='$desc' where soil_id='$soil_id'";
                $Model->execute($up2);
                if(is_bool($result)){
...
//                $json = json_decode($data,true);
                ...
                $area = $json["data"]["area"];
                $up = "update soil set soil_area='$area' where sid='$sid'";
                $result = $Model->execute($up);
                if(is_bool($result)){
    ...
    public function del_soil($token="",$sid){
        ...
        switch($this->_method) {
            case 'post':{
                $del = "delete from soil where sid='$sid'";
                $result = $Model->execute($del);
                if(is_bool($result)){
                    $this->response(retmsg(-1,null,"删除失败"),'json');
                }else{
                    $sql = "select soil_id from soil where sid='$sid'";
                    $result2 = $Model->query($sql);
                    $sid2 = $result2[0]["soil_id"];
                    $del2 = "delete from rent_soil where soil_id='$sid2'";
                    $Model->execute($del2);
    ...
    public function transfer_soil($token,$sid){
        ...
        switch($this->_method) {
            case 'get':{
                ...
                $sql2 = "select * from `soil` where sid='$sid'";
                $reslut2 = $Model->query($sql2);
                $soil_id = $reslut2[0]["soil_id"];
                $soil_area = $reslut2[0]["soil_area"];
                $soil_photo = $reslut2[0]["soil_photo"];
                $time = date("Y-m-d H:i:s");
                $sql3 = "select sid,trans_status from transfer where sid='$sid'";
                $re3 = $Model->query($sql3);
                if($re3){
                    $this->response(retmsg(-1,null,"该土地已被转让"),'json');
                }else{
                    $insert = "insert into transfer(trans_out,nickname_out,sid,soil_id,soil_area,soil_photo,apply_trans_time) values ("."'".
                        safe($user_phone)."','".safe($nickname)."',".safe($sid).",'".safe($soil_id)."','".safe($soil_area)."','".
                        safe($soil_photo)."','".safe($time)."'".")";
                    $re = $Model->execute($insert);
                    $up_first = "update soil set status_desc='转让待租赁' where sid='$sid'";
                    $Model->execute($up_first);
                    $up_second = "update rent_soil set soil_status='0',verify_desc='转让待租赁' where soil_id='$soil_id'";
                    $Model->execute($up_second);
                    $update = "update userinfor set soil_nums=soli_nums-1 where user_phone='$user_phone'";
                    $Model->execute($update);
                    $update2 = "update wallet set balance=balance+100 where token='$soil_photo'";
    ...
    public function all_transfer_soil(){
        ...
        switch($this->_method) {
            case 'get':{
                $sql = "select * from transfer where trans_status='待租赁'";
                $reslut = $Model->query($sql);
                $data = array();
                $data["subitem"] = $reslut;
                if(is_bool($reslut)){
    ...
    public function make_a_deal($token,$sid){//其他用户提交转租申请，进行土地权限转让
        ...
        switch($this->_method) {
            case 'post':{
                $sql = "select user_phone,nickname from userinfor where token='$token'";
                $re = $Model->query($sql);
                $sql2 = "select soil_id from soil where sid='$sid'";
                $re2 = $Model->query($sql2);
                $soil_id = $re2[0]["soil_id"];
                $user = $re[0]["user_phone"];
                $time = date("Y-m-d H:i:s");
                $name = $re[0]["nickname"];
                $update = "update soil set rent_to_who='$token',soil_status=0,status_desc='待审核' where sid='$sid'";
                $reslut = $Model->execute($update);
                $update2 = "update transfer set trans_in='$user',nickname_in='$name',trans_out_time='$time',trans_status='已租赁'  where sid='$sid'";
                $reslut2 = $Model->execute($update2);
                $update3 = "update userinfor set soil_nums=soil_nums+1 where token='$token'";
                $Model->execute($update3);
                $update4 = "update rent_soil set user_phone='$user',verify_status=0,verify_desc='待审核',unpass_cause='',verify_time='' where soil_id='$soil_id'";
                $Model->execute($update4);
                $update5 = "update wallet set balance=balance-100 where token='$token'";
