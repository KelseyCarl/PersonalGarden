...
class UserController extends RestController{
    public function query_user($token="",$uid="",$uname="",$page="1",$pagesize="10"){
        ...
        switch($this->_method) {
            case 'get':{
                ...
                $l = ($page-1)*$pagesize;
                if($where == "") {
                    $sql1 = "select * from userinfor order by login_time desc limit ".$l.",".$pagesize;
                }else{
                    $sql1 = "select * from userinfor  where  ".$where."  order by login_time desc  limit ".$l.",".$pagesize;
                    $m='1';
                }
    ...
    public function reset_pass($token="",$id){
        ...
        switch($this->_method) {
            case 'post':{
                //重置密码为：123456
                $up = "update userinfor set login_pass='123456' where user_id='$id'";
    ...
    public function one_user($token="",$id){
        ...
        switch($this->_method) {
            case 'get':{
                $sql = "select * from userinfor where user_id='$id'";
                $result = $Model->query($sql);
                if(is_bool($result)){
