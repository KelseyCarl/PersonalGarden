...
//                $json = json_decode($data,true);
                ...
                if(is_bool($re)){
                    ...
                    if($re == null){
                        ...
                        $up = "update admin set login_time='$time' where admin_account='$user'";
                        $Model->execute($up);
                        if($pswd != $pass){
    ...
    public function admin($token="",$uid="",$uname="",$page="1",$pagesize="10"){
        ...
        switch($this->_method) {
            case 'get':{
                ...
                $l = ($page-1)*$pagesize;
                if($where == "") {
                    $sql1 = "select * from admin order by login_time desc limit ".$l.",".$pagesize;
                }else{
                    $sql1 = "select * from admin  where  ".$where."  order by login_time desc  limit ".$l.",".$pagesize;
                    $m='1';
                }
...
//                $json = json_decode($data,true);
                ...
                if($re){
                    ...
                    $insert = "insert into admin(admin_account,admin_pass,token,name,privilege) values
("."'".safe($account)."','".safe($pass)."','".safe($token1)."','".safe($name)."','".safe($privilege)."'".")";
    ...
    public function reset_pass($token="",$id){
        ...
        switch($this->_method) {
            case 'post':{
                //重置密码为：123456
                $up = "update admin set admin_pass='123456' where id='$id'";
...
//                $json = json_decode($data,true);
                ...
                $up = "update admin set admin_account='$mobile',name='$uname' where admin_account='$userid'";
                $result = $Model->execute($up);
                if(is_bool($result)){
