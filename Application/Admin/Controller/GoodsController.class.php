...
class GoodsController extends RestController{
    public function all_goods($token="",$type="",$page="1",$pagesize="10"){
        ...
        switch($this->_method) {
            case 'get':{
                ...
                $sql = "select * from garden_items";
                $result = $Model->query($sql);
                $n = count($result);
                $m = ceil($n/$pagesize);
                $l = ($page-1)*$pagesize;
                if($where == "") {
                    $sql1 = "select * from garden_items order by apply_time desc limit ".$l.",".$pagesize;
                }else{
                    $sql1 = "select * from garden_items  where  ".$where."  order by apply_time desc  limit ".$l.",".$pagesize;
                    $m='1';
                }
    ...
    public function my_sale($token="",$id){//用户的id
        ...
        switch($this->_method) {
            case 'get':{
                $sql1 = "select nickname from garden_userinfor where user_id='$id'";
...
//              $data = '{"data":{"item_id":16,"is_verify":1,"unpass_cause":""}}';
                ...
                $up = "update garden_items set is_verify='$verify',status_desc='$desc',unpass_cause='$unpass',verify_time='$time' where id='$item_id'";
...
//                $json = json_decode($data,true);
                ...
                $up = "update garden_items set item_name='$itemname',item_desc='$itemdesc',item_amount='$amount',item_price='$price' where id='$id'";
                $result = $Model->execute($up);
                if(is_bool($result)){
    ...
    public function del_goods($token="",$id){
        ...
        switch($this->_method) {
            case 'post':{
                $del = "delete from garden_items where id='$id'";
