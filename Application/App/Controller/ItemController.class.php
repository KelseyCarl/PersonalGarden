...
class ItemController extends RestController{
    public function add_to_cart($token,$id){//添加到购物车
        ...
        switch($this->_method) {
            case 'get':{
                $sql = "select * from garden_cart where token='$token' and item_id='$id'";
                $reslut = $Model->query($sql);
                if($reslut == null){//没有添加过，则进行添加
                    $insert = "insert into garden_cart(token,item_id) values("."'".safe($token)."',".safe($id).")";
    ...
    public function cart_num($token){//添加到购物车
        ...
        switch($this->_method) {
            case 'get':{
                $sql1  = "select count(token) as total from garden_cart where token='$token'";
    ...
    public function show_cart($token){//我的购物车
        ...
        switch($this->_method) {
            case 'get':{
                $sql = "select * from garden_cart where token='$token' order by item_id desc";
                $result = $Model->query($sql);
                $data = null;
                for($i = 0;$i < count($result);$i++){
                    $item_id = $result[$i]["item_id"];
                    $sql2 = "select * from garden_items where is_verify=1 and id='$item_id'";
    ...
    public function del_cart_item($token="",$id){
        ...
        switch($this->_method) {
            case 'post':{
                $del = "delete from garden_cart where id='$id'";
    ...
    public function del_collect_item($token="",$id){
        ...
        switch($this->_method) {
            case 'post':{
                $del = "delete from garden_collect where id='$id'";
                $re = $Model->execute($del);
                if(is_bool($re)){
    ...
    public function collect($token,$id){//添加到我的收藏
        ...
        switch($this->_method) {
            case 'get':{
                $sql = "select * from garden_collect where token='$token' and item_id='$id'";
                $reslut = $Model->query($sql);
                if($reslut == null){//没有收藏过，则进行收藏
                    $insert = "insert into garden_collect(token,item_id) values("."'".safe($token)."',".safe($id).")";
                    $re = $Model->execute($insert);
                    if(is_bool($re)){
    ...
    public function show_collect($token){//我的收藏
        ...
        switch($this->_method) {
            case 'get':{
                $sql = "select * from garden_collect where token='$token' order by item_id desc";
                $result = $Model->query($sql);
                $data = null;
                for($i = 0;$i < count($result);$i++){
                    $item_id = $result[$i]["item_id"];
