...
//                $data = '{"data":{"pay_money":"5","item_id":7,"token":"8cbfacea76e34f6bf956afb40fc8e384"}}';
                ...
                $sql0 = "select item_name,item_type from garden_items where id='$item_id'";
                $re0 = $Model->query($sql0);
                $item_name = $re0[0]["item_name"];
                $item_type = $re0[0]["item_type"];
                $token = $json["data"]["token"];
                //查询购买者的信息
                $buyer = "select user_phone,nickname from garden_userinfor where token='$token'";
                $buy = $Model->query($buyer);
                //支付过后减去钱包中的余额现金
                $sql = "select balance from garden_wallet where token='$token'";
                $reslut = $Model->query($sql);
                if($money > $reslut[0]["balance"]){
                    $this->response(retmsg(-1,null,"您的余额已不足"),'json');
                }else{
                    //余额足够就进行支付，更新钱包里面的收支数据：balance
                    $update = "update garden_wallet set balance=balance-'$money' where token='$token'";
                    $re = $Model->execute($update);
                    //更新商品表中对应商品的库存量和售卖总数量
                    $update2 = "update garden_items set item_amount=item_amount-1,item_sale_out=item_sale_out+1 where id='$item_id'";
                    ...
                    $insert1 = "insert into `garden_order`(order_id,buyer_phone,buyer_name,item_id,buy_amount,order_status,order_status_desc,order_money,order_time,item_name,item_type) values ("."'".safe($order_id)."','"
                        .safe($buyer_phone)."','".safe($buyer_name)."','".safe($item_id)."','".safe($buy_account)."','"
                        .safe($order_status)."','".safe($order_status_desc)."','".safe($order_money)."','".safe($order_time)."','".safe($item_name)."','".safe($item_type)."'".")";
...
//                $json = json_decode($data,true);
                ...
                //更新商品表中对应商品的库存量和售卖总数量
                $update2 = "update garden_items set item_amount=item_amount-'$item_amount',item_sale_out=item_sale_out+'$item_amount' where id='$item_id'";
                $Model->execute($update2);
                //查询购买者的信息
                $buyer = "select user_phone,nickname from garden_userinfor where token='$token'";
                $buy = $Model->query($buyer);
...
//                $json = json_decode($data,true);
                ...
                if($total_pay > $reslut[0]["balance"]){
                    ...
                    //余额足够就进行支付，更新钱包里面的收支数据：balance
                    $update = "update garden_wallet set balance=balance-'$total_pay' where token='$token'";
                    $re = $Model->execute($update);
                    if (is_bool($re)) {
                        $this->response(retmsg(-1, null, "支付失败"), 'json');
                    } else {
